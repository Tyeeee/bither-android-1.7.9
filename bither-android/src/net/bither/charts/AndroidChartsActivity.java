...
import net.bither.charts.view.BOLLMASlipCandleStickChart;
import net.bither.charts.view.CandleStickChart;
import net.bither.charts.view.ColoredSlipStickChart;
import net.bither.charts.view.GridChart;
import net.bither.charts.view.LineChart;
import net.bither.charts.view.MACDChart;
import net.bither.charts.view.MACandleStickChart;
import net.bither.charts.view.MASlipCandleStickChart;
import net.bither.charts.view.MASlipStickChart;
import net.bither.charts.view.MAStickChart;
import net.bither.charts.view.MarketDepthChart;
import net.bither.charts.view.MinusStickChart;
import net.bither.charts.view.PieChart;
import net.bither.charts.view.SlipAreaChart;
import net.bither.charts.view.SlipBandAreaChart;
import net.bither.charts.view.SlipCandleStickChart;
import net.bither.charts.view.SlipLineChart;
import net.bither.charts.view.SlipMinusStickChart;
import net.bither.charts.view.SlipStickChart;
import net.bither.charts.view.SpiderWebChart;
import net.bither.charts.view.StickChart;
...
public class AndroidChartsActivity extends Activity implements
	...
	List<DateValueEntity> marketdepth2;
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		llDetail = (LinearLayout) findViewById(R.id.ll_display_detail);
		layoutParams = (FrameLayout.LayoutParams) this.llDetail
				.getLayoutParams();
		initVOL();
		initOHLC();
		initVOLC();
	...
	private void initMarketDepth2() {
		List<DateValueEntity> dv = new ArrayList<DateValueEntity>();
		this.marketdepth2 = new ArrayList<DateValueEntity>();
		for (int i = 100; i >= 0; i--) {
			dv.add(new DateValueEntity(i, 3800 + i * 10));
		}
		for (int i = 0; i < 100; i++) {
			dv.add(new DateValueEntity(i, 3900 + i * 10));
		}
		this.marketdepth2.addAll(dv);
	}

	private void initMarketDepth() {
		this.marketDepthChart = (MarketDepthChart) findViewById(R.id.marketdepthchart);
		// this.marketDepthChart.setViewTouchEventResponse(this);

		List<LineEntity<DateValueEntity>> lines = new ArrayList<LineEntity<DateValueEntity>>();
		LineEntity<DateValueEntity> MA10 = new LineEntity<DateValueEntity>();
		MA10.setTitle("LOW");
		MA10.setLineColor(Color.RED);
		MA10.setLineData(marketdepth2);
		lines.add(MA10);

		marketDepthChart.setAxisXColor(Color.LTGRAY);
		marketDepthChart.setAxisYColor(Color.LTGRAY);
		marketDepthChart.setBorderColor(Color.LTGRAY);
		marketDepthChart.setLongitudeFontSize(14);
		marketDepthChart.setLongitudeFontColor(Color.WHITE);
		marketDepthChart.setLatitudeColor(Color.GRAY);
		marketDepthChart.setLatitudeFontColor(Color.WHITE);
		marketDepthChart.setLongitudeColor(Color.GRAY);

		marketDepthChart.setMaxValue(100);
		marketDepthChart.setMinValue(0);
		// marketDepthChart.setDisplayFrom(0);
		marketDepthChart.setDisplayNumber(this.marketdepth2.size() - 1);
		marketDepthChart.setMinDisplayNumber(this.marketdepth2.size() - 1);
	...
	public void notifyTouchContentChange(Object[] objs) {
		tvOrder.setText(objs[0].toString());
		tvPrice.setText(objs[1].toString());
		tvVolume.setText(objs[2].toString());
